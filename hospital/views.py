...
def patient_signup_view(request):
    ...
    if request.method=='POST':
        print('function is calles in post on signup =======================')
        userForm=forms.PatientUserForm(request.POST)
        patientForm=forms.PatientForm(request.POST,request.FILES)
        print(patientForm.is_valid())
        print(userForm.is_valid())
        if userForm.is_valid() and patientForm.is_valid():
            print('function is calles in inside the validation =======================')   
            user=userForm.save()
            print(user)
            user.set_password(user.password)
            user.save()
...
def error_404_view(request,exception):
    print("function is callde")
    return render(request,'hospital/404.html')

def user_data_profile(request,pk):
    try:
        ...
        return render(request,'hospital/notfound.html')
